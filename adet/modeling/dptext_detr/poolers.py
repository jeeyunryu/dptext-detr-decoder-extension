...
def save_tensor_as_jpg(tensor, filename):
    ...
    if tensor.ndim == 3 and tensor.shape[0] in [1, 3]:  # Single channel or 3 channels
        tensor = tensor.transpose(1, 2, 0)  # Convert to HWC format
    elif tensor.ndim == 3 and tensor.shape[0] > 3:  # More than 3 channels
        tensor = tensor[0]  # Use only the first channel
...
def rotate_features(region):
    region_rotated = torch.rot90(region, 1, [2, 3])
    return region_rotated
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        if pooler_type == "ROIAlign":
            ...
            self.level_poolers_rotation = nn.ModuleList(
                ROIAlign(
                    (128, 32), spatial_scale=scale, sampling_ratio=sampling_ratio, aligned=True
                )
                for scale in scales
            )
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        assert_fx_safe(
            len(box_lists) == x[0].size(0), # batch size
        ...
        output = _create_zeros(pooler_fmt_boxes, num_channels, self.output_size[0], self.output_size[1], x[0])

        for level, pooler in enumerate(self.level_poolers):
            inds = nonzero_tuple(level_assignments == level)[0]
            pooler_fmt_boxes_level = pooler_fmt_boxes[inds] ## ? 
            feature_map_level = x[level] # res3, 4, 5, 6 각각 
            # if torch.isnan(feature_map_level).any():
            #     print(f'feature_map_level has nan!!: {feature_map_level}')
            #     import sys;sys.exit()
            
            for i, box in enumerate(pooler_fmt_boxes_level):
                
                batch_idx, x0, y0, x1, y1 = box
                width = x1 - x0
                height = y1 - y0
                if height > 2 * width:
                    # Use the rotation-aware ROIAlign for tall boxes

                    rotated_pooled_feature = self.level_poolers_rotation[level](feature_map_level, box.unsqueeze(0))
                    # Rotate the output feature back
                    # save_tensor_as_jpg(rotated_pooled_feature, f"rotated_pooled_feature_{i}.jpg")
                    rotated_pooled_feature = rotate_features(rotated_pooled_feature)
                    output[inds[i]] = rotated_pooled_feature
                else:
                    # Normal ROI pooling
                    pooled_feature = pooler(feature_map_level, box.unsqueeze(0))

                    output[inds[i]] = pooled_feature
